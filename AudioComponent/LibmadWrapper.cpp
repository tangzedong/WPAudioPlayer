...
static inline int _ReadNextFrame( MP3FileHandle* mp3 )
{
	do
	{
		if( mp3->stream.buffer == 0 || mp3->stream.error == MAD_ERROR_BUFLEN )  //缓冲区为空或者是正剩下残帧
		{
			int inputBufferSize = 0;
			if( mp3->stream.next_frame != 0 )  //有残帧的情况
			{
				int leftOver = mp3->stream.bufend - mp3->stream.next_frame;  //计算缓冲区内残存的数据帧的长度
				for( int i = 0; i < leftOver; i++ )
					mp3->inputBuffer[i] = mp3->stream.next_frame[i];  //复制残存的数据帧到缓冲区的起始位置
				int readBytes = _Sread( mp3->inputBuffer + leftOver, 1, INPUT_BUFFER_SIZE - leftOver, mp3); //从文件中读取数据填满缓冲区
				if( readBytes == 0 ) //文件末尾
					return 0;
			...
			mad_stream_buffer( &mp3->stream, mp3->inputBuffer, inputBufferSize );  //缓冲区绑定
